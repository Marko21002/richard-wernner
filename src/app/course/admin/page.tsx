...
const CoursesOverviewPanel = () => {
  ...
  const courseId = "banking-masterclass";

  useEffect(() => {
    const loadThumbnail = async () => {
      try {
        const response = await fetch(`/api/course/${courseId}/thumbnail`);
        if (response.ok) {
        ...
        console.error("Error loading course thumbnail:", error);
      }
    };
              ...
              <tr className="border-b border-slate-100 last:border-b-0">
                <td className="px-3 py-2">
                  {courseThumbnail ? (
                    <img
                      src={courseThumbnail}
                      alt={CourseDefaults.heading}
                      className="h-12 w-20 object-cover rounded-sm border border-slate-200"
                    />
                  ) : (
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="max-w-2xl text-sm text-slate-600">
          This section will eventually let you attach workbooks, Q&amp;A replays
          and other extras to your courses. For now it&apos;s a visual preview
          and not part of the core admin workflow.
        </p>
      </header>
